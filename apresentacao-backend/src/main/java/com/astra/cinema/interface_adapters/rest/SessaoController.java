...
public class SessaoController {
    ...
    /**
     * Retorna mapa completo de assentos com disponibilidade para uma sessão
     */
    @GetMapping("/{id}/assentos")
    public ResponseEntity<Map<String, Object>> buscarAssentos(@PathVariable Integer id) {
        try {
            Sessao sessao = sessaoRepositorio.obterPorId(new SessaoId(id));
            if (sessao == null) {
                return ResponseEntity.notFound().build();
            ...
            // Converter Map<AssentoId, Boolean> para Map<String, Boolean>
            Map<String, Boolean> mapaAssentos = sessao.getMapaAssentosDisponiveis()
                    .entrySet()
                    .stream()
            ...
            resposta.put("sessaoId", sessao.getSessaoId().getId());
            resposta.put("capacidade", sessao.getCapacidade());
            resposta.put("assentos", mapaAssentos);
    ...
    /**
     * Reserva assentos específicos para uma sessão, garantindo sincronização com o backend
     */
    @PostMapping("/{id}/assentos/reservar")
    public ResponseEntity<Map<String, Object>> reservarAssentos(
        ...
        try {
            Sessao sessao = sessaoRepositorio.obterPorId(new SessaoId(id));
            if (sessao == null) {
                response.put("mensagem", "Sessão não encontrada");
                response.put("status", "erro_nao_encontrada");
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(response);
            }

                List<String> assentosEntrada = request.assentos == null ? Collections.emptyList() : request.assentos;

                List<String> assentosSolicitados = assentosEntrada.stream()
                    .map(valor -> valor != null ? valor.trim().toUpperCase() : null)
                    .filter(valor -> valor != null && !valor.isBlank())
                    .toList();

            if (assentosSolicitados.isEmpty()) {
                response.put("mensagem", "Informe pelo menos um assento para reservar");
                response.put("status", "erro_validacao");
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
            }

            Map<AssentoId, Boolean> mapaAtual = sessao.getMapaAssentosDisponiveis();
            List<String> inexistentes = assentosSolicitados.stream()
                    .filter(codigo -> !mapaAtual.containsKey(new AssentoId(codigo)))
                    .toList();

            if (!inexistentes.isEmpty()) {
                response.put("mensagem", "Assentos inexistentes: " + String.join(", ", inexistentes));
                response.put("status", "erro_validacao");
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
            }

            List<String> jaOcupados = new ArrayList<>();
            for (String codigo : assentosSolicitados) {
                AssentoId assentoId = new AssentoId(codigo);
                if (!sessao.assentoDisponivel(assentoId)) {
                    jaOcupados.add(codigo);
                }
            }

            if (!jaOcupados.isEmpty()) {
                response.put("mensagem", "Assentos já ocupados: " + String.join(", ", jaOcupados));
                response.put("status", "erro_conflito");
                return ResponseEntity.status(HttpStatus.CONFLICT).body(response);
            }

            for (String codigo : assentosSolicitados) {
                sessao.reservarAssento(new AssentoId(codigo));
            }

            Sessao sessaoAtualizada = sessaoRepositorio.salvar(sessao);
            int disponiveis = calcularAssentosDisponiveis(sessaoAtualizada);

            Map<String, Boolean> mapaAssentos = sessaoAtualizada.getMapaAssentosDisponiveis()
                    .entrySet()
                    .stream()
                    .collect(Collectors.toMap(
                            entry -> entry.getKey().getValor(),
            ...
            response.put("mensagem", "Assentos reservados com sucesso");
            response.put("sessaoId", sessaoAtualizada.getSessaoId().getId());
            response.put("assentosReservados", assentosSolicitados);
            response.put("assentosDisponiveis", disponiveis);
            response.put("assentosOcupados", sessaoAtualizada.getCapacidade() - disponiveis);
            response.put("assentos", mapaAssentos);
            response.put("clienteId", request.clienteId);
            response.put("origem", request.origem);
            ...
            response.put("mensagem", "Erro ao reservar assentos: " + e.getMessage());
