...
public class ProgramacaoService {
    ...
    private void validarConflitosDeHorario(List<Sessao> sessoes) {
        ...
        Map<String, List<Sessao>> sessoesPorSala = sessoes.stream()
            .collect(Collectors.groupingBy(Sessao::getSala));

        // Para cada sala, verifica conflitos
        for (Map.Entry<String, List<Sessao>> entrada : sessoesPorSala.entrySet()) {
            ...
            for (int i = 0; i < sessoesDaSala.size() - 1; i++) {
                ...
                if (!proximaSessao.getHorario().after(fimSessaoAtual.getTime())) {
                    throw new IllegalStateException(
                        String.format("Conflito de horário na sala %s: Sessão #%d termina depois do início da Sessão #%d",
                            sala,
